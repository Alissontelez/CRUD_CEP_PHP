...
$seleciona = mysqli_query($user_db, "SELECT * FROM coment_kuest ORDER BY id DESC");
$i = 1;
...
<h5><?php echo $sel["Comentário"]; ?> - <?php echo $sel["Data"];
    if ($usuárioS === $sel["Usuário"]){
        ?> <button type="button" class="btn btn-primary editCO-kuest" id="<?php echo $sel["id"]; ?>">Editar</button> 
        <button type="button" class="btn btn-danger del-kuest" id= "<?php echo $sel["id"]; ?>">Deletar</button>
    <?php }
?></h5>
<br>
<?php endforeach;}else{
